...
> **⚠️ 실행 환경 안내**: 이 장의 모든 코드는 TensorFlow/Keras가 필요하며, 이 책의 검증 환경에는 미설치라 **전 코드가 `no-run`**입니다. 직접 실행하려면 다음을 설치하세요.
> ```bash
> pip install tensorflow
> ```
> API 시그니처는 Keras 공식 기준으로 작성했습니다. 층을 쌓는 발상은 18장과 동일하니, 새 층의 **역할과 인자**에 집중하세요.
...
callbacks = [
    EarlyStopping(monitor="val_loss", patience=3,
                  restore_best_weights=True),          # 3에폭 개선 없으면 중단
